...
def plot_graphics_sigmoid(X, y):
    ALPHA = (1e-6, 1e-4, 1e-2)
    THRESHOLD = (1e-5, 1e-4, 1e-3)
    Q = []
    for alpha, threshold in zip(ALPHA, THRESHOLD):
        gd = GradientDescent(alpha=alpha, threshold=threshold, loss=sigmoid_loss)
        q = gd.fit(X, y)
        Q.append(q)
        print(len(q))
    for q, alpha, threshold in zip(Q, ALPHA, THRESHOLD):
        plt.plot(q, label="alpha = {:.0e}, threshold = {:.0e}".format(alpha, threshold))
        plt.legend()
    plt.show()


def plot_graphics_log(X, y):
    ALPHA = (1e-6, 1e-4, 1e-3)
    THRESHOLD = (1e-4, 1e-5, 1e-6)
    Q = []
    for alpha, threshold in zip(ALPHA, THRESHOLD):
...
def plot_graphics_sgd_sigmoid(X, y):
    ALPHA = [1e-6, 1e-4, 1e-2]
    K = [1, 10, 50]
    for k in K:
        for a in ALPHA:
            sgd = SGD(alpha=a, k=k, loss=sigmoid_loss, n_iter=10000)
            q = sgd.fit(X, y)
            plt.plot(q, label="alpha = {:.0e}, k = {}".format(a, k))
            plt.legend()
    plt.show()


def plot_graphics_sgd_log(X, y):
    ALPHA = [1e-6, 1e-4, 1e-2]
    K = [1, 10, 50]
    for k in K:
        for a in ALPHA:
            sgd = SGD(alpha=a, k=k, loss=log_loss, n_iter=10000)
            q = sgd.fit(X, y)
            plt.plot(q, label="alpha = {:.0e}, k = {}".format(a, k))
            plt.legend()
    plt.show()
...
def main():
    ...
    # test_algorithms(X, y)
    # plot_graphics_sigmoid(X, y)
    # plot_graphics_log(X, y)
    # plot_graphics_sgd_log(X, y)
